...
static inline quat_t quatrealdiv(quat_t a, float b);
...
static inline quat_t quatinverse(quat_t q) { return quatrealdiv(quatconj(q), quatsq(q)); }
...
static inline bool quatiszero(quat_t q) { return q.r == 0 && vec3iszero(q.i); }
static inline bool quatalmostzero(quat_t q, float epsilon) { return fabsf(q.r) < epsilon && vec3almostzero(q.i, epsilon); }
...
static inline quat_t quatmix(quat_t a, quat_t b, float t) {
	return quat(a.r + (b.r - a.r) * t, vec3mix(a.i, b.i, t));
}
